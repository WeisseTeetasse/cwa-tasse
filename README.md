...
This fork currently carries these local changes on top of upstream CWA:

1. Hardcover State Sync.
   - Adds per-user syncing between Hardcover Currently Reading status and one normal CWA shelf.
...
6. Magic Shelf refresh behavior.
...
7. Multiple Kobo device tokens per user.
...
Recommended future upstream update flow:
...
If conflicts are difficult, create a fresh branch from upstream and replay the local patch
commits one by one, resolving conflicts as they appear.
